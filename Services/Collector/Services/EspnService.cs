...
namespace Wingrid.Services.Collector.Services
{
    ...
    {   
        ...
        {
            TeamsResponse response = await _client.GetFromJsonAsync<TeamsResponse>($"{ESPN_CORE_API}/teams?limit=32") ?? throw new Exception("No response from ESPN.");
        ...
        {   
            ...
            for (var w = 0; w < 18; w++)
            {
                EventsResponse? response = await _client.GetFromJsonAsync<EventsResponse>($"{ESPN_SITE_API}/scoreboard?dates={season}&seasontype=2&week={w + 1}") ?? throw new Exception("No response from ESPN.");
        ...
        {
            EventsResponse? response = await _client.GetFromJsonAsync<EventsResponse>($"{ESPN_SITE_API}/scoreboard?dates={year}&seasontype=2&week={week}") ?? throw new Exception("No response from ESPN.");
